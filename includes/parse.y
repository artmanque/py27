...
%{ 
	...
	std::vector<std::vector<Node*>> tests;
	std::vector<std::vector<std::vector<Node*>*>> suites;
	...
	Literal* none = new IntLiteral(INT_MAX);
%}
...
compound_stmt // Used in: stmt
	:{ SymbolTableManager::getInstance().funcDepth += 1; elif_Depth++;	tests.push_back(std::vector<Node*>()); suites.push_back(std::vector<std::vector<Node*>*>());} if_stmt     { $$ = $2; }       
	| while_stmt  { $$ = $1; }
	| for_stmt    { $$ = NULL; }
	| try_stmt    { $$ = NULL; }
	| with_stmt   { $$ = NULL; }
...
if_stmt // Used in: compound_stmt
	...
	{
		tests[elif_Depth].insert(tests[elif_Depth].begin(),$2);
		suites[elif_Depth].insert(suites[elif_Depth].begin(),$4);
		suites[elif_Depth].push_back($8);
		Node* ifelse = new IfNode(tests[elif_Depth], suites[elif_Depth], true); //pool.add($$);
		SymbolTableManager::getInstance().funcDepth -= 1;
		if(SymbolTableManager::getInstance().funcDepth == 0){
	...
	| IF test COLON suite star_ELIF
	{
		tests[elif_Depth].insert(tests[elif_Depth].begin(),$2);
		suites[elif_Depth].insert(suites[elif_Depth].begin(),$4);
		Node* elif = new IfNode(tests[elif_Depth], suites[elif_Depth], false); //pool.add($$);
		SymbolTableManager::getInstance().funcDepth -= 1;
		if(SymbolTableManager::getInstance().funcDepth == 0){
...
star_ELIF // Used in: if_stmt, star_ELIF
	: star_ELIF ELIF test {tests[elif_Depth].push_back($3);} COLON suite {suites[elif_Depth].push_back($6);}
	| %empty 
	;
